...
fig, (tke, speed)=plt.subplots(2,1,figsize=(10,12))
fig.subplots_adjust(hspace=0.4)
...
speed.plot(pasture_wind_dynamics.index, pasture_wind_dynamics.WS, label= 'Pastture', color='black')
speed.set_title('Wind Speed')
speed.legend()
speed.yaxis.set_ticks([0,2,4,6])
speed.set_ylabel('m/s2')
...
e,f = np.polyfit(soil_f.swc, soil_f.ts, 1)
g,h = np.polyfit(soil_p.swc, soil_p.ts,1)

fig,(f, p) = plt.subplots(1,2, figsize=(8,5))
fig.suptitle('Correlation between SWC & TS', fontsize=14)
fig.subplots_adjust(hspace=0.25)

f.plot(soil_f.swc, soil_f.ts,'sg')
f.plot(soil_f.swc, 0.12*soil_f.swc + 11.12, color='royalblue')
f.text(39, 16.6,'r = 0.71', fontsize=15)
f.set_title('Forest')

p.plot(soil_p.swc, soil_p.ts,'sg')
p.plot(soil_p.swc, g*soil_p.swc + h, color='royalblue')
p.text(7,18.75, 'r = 0.35', fontsize=15)
p.set_title('Pasture')

#Corr between SCW and Surface Fluxes (LE & H)

#Between TS and LE

cor_5 = pd.concat([soil_p.ts,surface_energy_p.iloc[:,0]], axis=1)
cor_6 = pd.concat([soil_f.ts,surface_energy_f.iloc[:,0]], axis=1)
...
i,j = np.polyfit(cor_5.ts, cor_5.iloc[:,1], 1)
k,l = np.polyfit(cor_6.ts, cor_6.iloc[:,1],1)

fig,(f, p) = plt.subplots(1,2, figsize=(8,5))
fig.suptitle('Correlation between TS & LE', fontsize=14)
fig.subplots_adjust(hspace=0.25)

f.plot(cor_5.ts, cor_5.iloc[:,1],'sb')
f.plot(cor_5.ts, i*cor_5.ts+j, color='crimson')
f.text(16.35, 400, 'r = 0.09', fontsize=15)
f.set_title('Pasture')

p.plot(cor_6.ts, cor_6.iloc[:,1],'sb')
p.plot(cor_6.ts, k*cor_6.ts+l, color='crimson')
p.text(16.25, 250, 'r = -0.46', fontsize=15)
p.set_title('Forest')


#TS and H

cor_5 = pd.concat([soil_p.ts,surface_energy_p.iloc[:,1]], axis=1)
cor_6 = pd.concat([soil_f.ts,surface_energy_f.iloc[:,1]], axis=1)
...
fig,(f, p) = plt.subplots(1,2, figsize=(8,5))
fig.suptitle('Correlation between TS & H', fontsize=14)
fig.subplots_adjust(hspace=0.25)

f.plot(cor_5.ts, cor_5.iloc[:,1],'sb')
f.plot(cor_5.ts, i*cor_5.ts+j, color='crimson')
f.text(16.35, 120, 'r = -0.14', fontsize=15)
f.set_title('Pasture')

p.plot(cor_6.ts, cor_6.iloc[:,1],'sb')
p.plot(cor_6.ts, k*cor_6.ts+l, color='crimson')
p.text(16, 250, 'r = -0.48', fontsize=15)
p.set_title('Forest')


#LE and H

cor_5 = pd.concat([surface_energy_p.iloc[:,0],surface_energy_p.iloc[:,1]], axis=1)
cor_6 = pd.concat([surface_energy_f.iloc[:,0],surface_energy_f.iloc[:,1]], axis=1)
...
f.plot(cor_5.iloc[:,0], i*cor_5.iloc[:,0]+j, color='crimson')
f.text(10, 175, 'r = 0.71', fontsize=15)
f.set_title('Pasture')

p.plot(cor_6.iloc[:,0], cor_6.iloc[:,1],'sb')
p.plot(cor_6.iloc[:,0], k*cor_6.iloc[:,0]+l, color='crimson')
p.text(10,320, 'r = 0.68', fontsize=15)
p.set_title('Forest')
